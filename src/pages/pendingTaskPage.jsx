...
export default function PendingTaskPage(){
    ...
    const fetchTasks = async ()=>{
        ...
        // console.log("Fetching tasks for user:", user.id);
            ...
            const {data, error} = await supabase.from("tasks").select("*").eq("user_id", user.id).eq("taskStatus", "pending").order("created_at", {ascending: false});
        ...
        <div className="w-[90%] mx-auto flex items-center justify-between lg:flex-row flex-col">
            <div className="flex items-center gap-2">
                <Link to="/dashboard/task" className="flex items-center gap-2">
                    <span className="text-2xl border-1 w-8 h-auto dark:border-red-600 flex items-center justify-center rounded my-4 text-gray-600 dark:text-red-600"><IoIosArrowRoundBack /></span>
                ...
                <span className="text-gray-900 dark:text-white text-base font-[sans]">Pending Tasks</span>
            </div>
                    ...
                    <div className="w-[95%] mx-auto my-10 h-full flex items-center justify-center lg:justify-start flex-wrap gap-6">
                        {paginatedPage.map((task, index)=> (
                            <div key={index} className="bg-white dark:bg-[#343a40] w-[300px] h-auto p-4 rounded-md relative">
                                <div className="absolute inset-0 w-full h-[10px] rounded-t-md bg-red-600"></div>
                                <div className="flex items-center justify-between font-[sans] text-sm font-normal my-2">
                                    <div className="flex items-center gap-1">
                                        ...
                                        <span>-</span>
                                        <span className="bg-red-300 text-red-900 py-0.5 p-1 rounded-md capitalize">{task.taskStatus}</span>
                                    </div>
                                ...
                                <div className="w-full flex items-center justify-between">
                                    <div className="mt-6 text-sm font-[sans]">
                                            ...
                                            <p className="flex items-center gap-1 font-semibold text-red-700 dark:text-cyan-40 text-sm"><FaRegClock /> {new Date(task.taskDueDate).toLocaleDateString("en-us", {
                                                year: "numeric",
                                                month: "short",
                                                day: "numeric"
                                            })}</p>
            ...
            <div className="bg-white dark:bg-[#24272F] h-auto rounded-md w-[500px] shadow-lg">
                ...
                <div className="p-2 mt-6">
                    ...
                    <div>
                        ...
                        <p className="font-[sans] text-base text-gray-700 dark:text-white font-semibold">{selectedTask.taskStatus}</p>
                    </div>
