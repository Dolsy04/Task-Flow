...
export default function DoneTaskPage(){
    ...
    const fetchTasks = async ()=>{
        ...
        // console.log("Fetching tasks for user:", user.id);
            ...
            const {data, error} = await supabase.from("tasks").select("*").eq("user_id", user.id).eq("taskStatus", "done").order("created_at", {ascending: false});
    ...
    return (<>
        ...
        <div className="w-[95%] mx-auto flex items-center justify-between lg:flex-row flex-col">
            <div className="flex items-center gap-2">
                <Link to="/dashboard/task" className="flex items-center gap-2">
                    ...
                    <span className="text-gray-600 text-base font-[sans] dark:text-red-600">Go back</span>
                </Link>
                <span className="text-gray-600 dark:text-red-600 text-base font-[sans]">/</span>
                <span className="text-gray-900 dark:text-white text-base font-[sans]">Finished Tasks</span>
            </div>
            <div className="border-2 border-gray-500 dark:border-white rounded-full w-full md:w-[50%] lg:w-[30%] flex items-center h-[40px] px-2 focus-within:border-blue-400">
                <IoSearch className="h-full w-[20px] rounded-full dark:text-white text-gray-600"/>
                    ...
                    <div className="w-[95%] mx-auto my-10 h-full flex items-center justify-center lg:justify-start flex-wrap gap-6">
                        {paginatedPage.map((task, index)=> (
                            <div key={index} className="bg-white dark:bg-[#343a40] w-[300px] h-auto p-4 rounded-md relative">
                                <div className="absolute inset-0 w-full h-[10px] rounded-t-md bg-[#00A63E]"></div>
                                <div className="flex items-center justify-between font-[sans] text-sm font-normal my-2">
                                    <div className="flex items-center gap-1">
                                        ...
                                        <span>-</span>
                                        <span className="bg-green-300 text-green-900 py-0.5 p-1 rounded-md capitalize">{task.taskStatus}</span>
                                    </div>
                                ...
                                <div className="pt-6 w-full flex items-center justify-between mb-6">
                                    <div className=" ">
                                        <h3 className="font-semibold tracking-wide text-sm dark:text-white text-gray-700 font-[sans]">{task.taskName.charAt(0).toUpperCase() + task.taskName.slice(1)}</h3>
                                ...
                                <div className="w-full flex items-center justify-between">
                                    <div className="mt-6 text-sm font-[sans]">
                                            <p className="text-gray-600 dark:text-white ">Due By</p>
        ...
        <div className="w-full h-screen fixed inset-0 bg-white/50 dark:bg-[#1a1d23d2] backdrop-blur-xs flex items-center justify-center z-50">
            <div className="bg-white dark:bg-[#24272F] h-auto rounded-md w-[500px] shadow-lg">
                <div className="flex items-center rounded-md justify-between shadow py-4 px-6">
                    <h3 className="text-lg font-[sans] font-semibold text-[#07011e] capitalize tracking-wide dark:text-white">Task Details</h3>
                ...
                <div className="p-2 mt-6">
                    <div>
                        ...
                        <p className=" font-[sans] text-base  dark:text-white text-gray-700  font-semibold">{selectedTask.taskName}</p>
                    </div>
                    <div>
